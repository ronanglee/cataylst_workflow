...
def main(**data: dict) -> tuple[bool, dict | None]:
    ...
    if ooh_descriptor_cutoffs[0] < ooh_descriptor < ooh_descriptor_cutoffs[1] and ul_2e > ul_2e_cutoff:
        add_entry(os.path.join(database_dir, "seperated_act_sel.json"), database)
        return True, None
    else:
        if ul_2e < ul_2e_cutoff:
            if ooh_descriptor < ooh_descriptor_cutoffs[0] or ooh_descriptor > ooh_descriptor_cutoffs[1]:
                run_logger(f"DISCARD - {structure}/{carbon_structure}/{dopant}{metal} - activity ul_2e {ul_2e} < {ul_2e_cutoff} eV and selectivity {ooh_descriptor} outside  [{ooh_descriptor_cutoffs[0]}, {ooh_descriptor_cutoffs[1]}] eV.", 
                str(__file__), 'error')
            else:
                run_logger(f"DISCARD - {structure}/{carbon_structure}/{dopant}{metal} - activity ul_2e {ul_2e} < {ul_2e_cutoff} eV",
                        str(__file__), 'error')
        else:
            run_logger(f"DISCARD - {structure}/{carbon_structure}/{dopant}{metal} - selectivity {ooh_descriptor} < {ooh_descriptor_cutoffs[0]} or > {ooh_descriptor_cutoffs[1]} eV.", 
            str(__file__), 'error')
        return False, None